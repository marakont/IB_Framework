## Intensity bioassay framework
## Example code for multiple dose-response curves (loop) &
## create summary graphs & tables
...
df <- import(here("data/example_data.csv")) # load data
...
model <- rstan::stan_model(here("src/Stan/example_model.stan"))
...
IB_model <- function(df_single) {
  ...
  if(modelcheck$not_converged > 0){
    fit <- sampling(model, data=data_stan, iter=10000, chains=4) # rhat < 1.01 for convergence
...
routput_lab <- lapply(ldf, IB_model) # model each bioassay
